...
let fumble = huskyGames2016.filter(function(game){return game.fumbles >= 1});
...
function mostYardsPassing(games1, games2){
    if(games1.passing_yards === undefined){
      return games2;
    }else if(games2.passing_yards === undefined){
      return games2;
    }
    if(games1.passing_yards > games2.passing_yards){
      return games1;
    }
    return games2;
}
...
function makeCombinedFilter(afunction, bfunction){
  let combinedFilter = function(game){
    return afunction(game) && bfunction(game);
  };
...
let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function(game){return game.fumbles >= 1});
