...
//  add a string with a int, it will convert int to a stirng the result will be 84
...
/* Create a variable `deck` that represents a deck of modern playing cards
   ...
   *array* of 52 elements, each of which is an Object with properties:
    ...
    You can log out the `deck` to check your work! */
      let deck = [];
      let s = ["hearts", "diamonds", "clubs", "spades"];
      for(let i = 2; i <= 14; i++){
        for(let j of s){
          let obj={suit: j, rank: i};
          deck.push(obj);
        }
      }
...
function containsQueenOfHearts(nums){
  for(let num of nums){
    if(num.suit == "hearts" && num.rank == 12){
      return true;
    }
  }
  return false;
...
/* Define a function `isFlush()` that takes in an array of "card" objects and
  ...
  function isFlush(nums){
    let firstSuit = nums[0].suit;
    for(let num of nums){
      if(firstSuit != num.suit){
        return false;
      }
    }
    return true;
